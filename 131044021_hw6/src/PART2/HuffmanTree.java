...
public class HuffmanTree
  ...
  public String decode(String codedMessage) {
       ...
       * @return
       */
      //my Function Encode Method
      public String encode(String message)
      {
          StringBuilder result = new StringBuilder();
          BinaryTree<HuffData> currentTree = huffTree;
          if(currentTree==null){
            System.out.println("Firstly you build tree. Null pointer. It exits -1");
            System.exit(-1);
          }
          if(message==null)
          {
            System.out.println("Your string is wrong. Null pointer. It exits -1");
            System.exit(-1);
          }
          for(int i =0;i < message.length();i++)
          {
            endcodeR(message,currentTree,i,result);
       ...
       * @return
       */
      private boolean endcodeR(String mes, BinaryTree<HuffData> tree,int index,StringBuilder st )
      {

          if(tree.isLeaf()){

            if(tree.getData().getSymbol().equals(mes.charAt(index)))
              ...
              boolean temp=false;
              st.append(0);
              boolean choose = endcodeR(mes, tree.getLeftSubtree(), index, st);
              ...
              if (choose == false) {
                ...
                if (st.length() > 0)
                  st.deleteCharAt(st.length() - 1);
                st.append(1);
                temp = endcodeR(mes, tree.getRightSubtree(), index, st);
                if (temp == false) {
                  st.deleteCharAt(st.length() - 1);
                }
              }
