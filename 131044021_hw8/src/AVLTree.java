...
public class AVLTree < E
  ...
  private void decrementBalance(AVLNode < E > node) {
    ...
    public E delete(E item) {
        decrease = false;
        //iki parametreli delete methodu
        root = deleteFunction( (AVLNode < E > ) root, item);
        //Return value from the public delete method.
        //Binary search tree protected data field
        return deleteReturn;
    }
     ...
     * @return
     */
    private AVLNode < E > deleteFunction(AVLNode < E > tempRoot, E item)
    {
      //null durumu check
      if (tempRoot == null) { // item is not in tree
          deleteReturn = null;
          return tempRoot;
      }


      //binary search tree mantigina gore ayni eleman eklenemez
      //ayni eleman mi diye check durumu
      if (item.compareTo(tempRoot.data) == 0)
      {
          // item is in the tree -- need to remove it
          deleteReturn = tempRoot.data;
          return findReplacementNode(tempRoot);
      }

      //parametre olarak alinan item root.data'dan buyukse durumu
      else if(item.compareTo(tempRoot.data) > 0)
      {
          //recursive kol
          tempRoot.right = deleteFunction( (AVLNode < E > ) tempRoot.right, item);
          //decrease check
          if (decrease)
          {
            //eger dengeli ise toplam yukseklik artmiyacak
            decrementBalance(tempRoot);

            if (tempRoot.balance < AVLNode.LEFT_HEAVY) {
              //left-right
              return rebalanceLeftR(tempRoot);
            }
            else {
              return tempRoot;
            }
      ...
      //parametre olarak alinan item root.data'dan kucukse durumu
      else{
          tempRoot.left = deleteFunction( (AVLNode < E > ) tempRoot.left, item);
          //decrease check
          if (decrease)
          {
            //eger dengeli ise toplam yukseklik azalmayacak.
            incrementBalance(tempRoot);
            if (tempRoot.balance > AVLNode.RIGHT_HEAVY) {
              //right-left
              return rebalanceRightL( (AVLNode < E > ) tempRoot);
            }
            else {
                return tempRoot;
            }
